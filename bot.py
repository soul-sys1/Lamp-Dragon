    ...
    if stat_value >= 95:
        messages = {
            ...
            "сон": [
                ...
                f"💤 {dragon_trait} сияет: 'Я прекрасно выспался!'"
            ],
            "настроение": [
            ...
            "аппетит": [
                ...
                f"🍪 'Спасибо, я больше не могу!' - говорит {dragon_trait} с улыбкой"
            ],
            "энергия": [
...
def format_stat_line(stat_name: str, stat_value: int) -> str:
    ...
    stat_names = {
        "кофе": "☕ Кофе",
        "сон": "💤 Бодрость",  # ИЗМЕНЕНО: теперь это "бодрость"
        "настроение": "😊 Настроение",
        "аппетит": "🍪 Сытость",  # ИЗМЕНЕНО: теперь это "сытость"
        "энергия": "⚡ Энергия",
...
async def cmd_status(message: types.Message):
    ...
    try:
        ...
        # ИЗМЕНЕНО: Теперь низкая бодрость (<30%) = хочет спать
        if dragon.stats.get("сон", 50) < 30:  # Бодрость <30% = хочет спать
            warnings.append("💤 Дракон с трудом держит глазки открытыми...")
        # ИЗМЕНЕНО: Теперь низкая сытость (<30%) = голоден
        if dragon.stats.get("аппетит", 50) < 30:  # Сытость <30% = голоден
