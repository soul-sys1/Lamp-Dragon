...
- Умные уведомления (1 раз в день)
"""
import asyncio
import logging
import random
...
class SmartNotifier:
    """Умная система уведомлений - отправляет одно утреннее уведомление в день"""
    
    def __init__(self):
        self.last_notification: Dict[int, datetime] = {}
        self.user_activity: Dict[int, datetime] = {}
    
    def should_send_notification(self, user_id: int) -> bool:
        """Проверяет, нужно ли отправлять утреннее уведомление"""
        now = datetime.now(timezone.utc)
        
        # Если пользователь был активен сегодня - не отправляем уведомление
        last_activity = self.user_activity.get(user_id)
        if last_activity and last_activity.date() == now.date():
            return False
        
        # Проверяем последнее уведомление
        last_notif = self.last_notification.get(user_id)
        if last_notif and last_notif.date() == now.date():
            return False
        
        # Проверяем время (утро: 8-10 утра)
        if 8 <= now.hour <= 10:
            return True
    ...
    def record_notification(self, user_id: int):
        """Записывает отправленное уведомление"""
        self.last_notification[user_id] = datetime.now(timezone.utc)
    ...
    def clear_old_entries(self):
        ...
        keys_to_delete = []
        for key, last_time in self.last_notification.items():
        ...
        keys_to_delete = []
        for key, last_time in self.user_activity.items():
            if last_time < week_ago:
                keys_to_delete.append(key)
        for key in keys_to_delete:
            del self.user_activity[key]
    ...
    def get_sleep_scene(dragon_name: str, action: str, book_title: str = None, book_content: str = None) -> str:
        if action == "read" and book_title and book_content:
            ...
            return (
                ...
                f"а он тихо мурлычет в ответ. Постепенно мурлыканье стихает, песне затихает, остаётся лишь тихое напевание. "
        ...
        try:
            if update and hasattr(update, 'message') and update.message:
                await update.message.answer(
                    ...
                    "Пожалуйста, попробуйте ещё раз или используйте команду /start",
                    parse_mode="HTML",
...
async def cmd_cancel(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "✅ Действие отменено.\n\n"
        "Вы можете начать заново с помощью кнопок меню.",
        parse_mode="HTML",
        reply_markup=get_main_keyboard()
    )
...
async def cmd_start(message: types.Message):
    try:
        ...
        welcome_text = (
            f"✨ Добро пожаловать в мир Кофейных Драконов, {escape_html(username)}! ✨\n\n"
            ...
            f"📋 ВОЗМОЖНОСТИ 7.0:\n"
            f"• 🎭 10 уникальных характеров с глубокой проработкой\n"
            f"• 🍪 Новая система кормления отдельным действием\n"
            f"• 📖 Только развёрнутые сцены - больше погружения\n"
            f"• 🛡️ Умная анти-спам система с жалобами дракона\n"
            f"• 📊 Выровненная статистика - удобный просмотр\n"
            f"• ⏰ Умные уведомления (1 раз в день утром)\n\n"
        )
        ...
        smart_notifier.record_activity(user_id)
        
        logger.info(f"Новый пользователь: {username} (ID: {user_id})")
        
    except Exception as e:
        logger.error(f"Ошибка в cmd_start: {e}")
        await message.answer("❌ Произошла ошибка при запуске бота.", parse_mode="HTML")
...
async def cmd_help(message: types.Message, state: FSMContext):
    try:
        ...
        help_text = (
            ...
            "🍪 НОВОЕ: КОРМЛЕНИЕ\n"
            "/feed - покормить дракона отдельно (повышает сытость)\n\n"
            
            "😴 СОН И ОТДЫХ\n"
            "/sleep - уложить дракона спать с разными сценами\n\n"
            
            "❤ УХОД И ЗАБОТА\n"
            "/coffee - приготовить кофе с добавками и угощениями\n"
            "/hug - обнять дракона в разных ситуациях\n"
            "/care - ухаживать за драконом\n\n"
            ...
            "⏰ УВЕДОМЛЕНИЯ\n"
            "• Одно утреннее уведомление в день (8-10 утра)\n"
            "• Только если вы не были активны сегодня\n\n"
        ...
        await message.answer("❌ Произошла ошибка при показе помощи.", parse_mode="HTML")
    ...
    try:
        ...
        if action == "commands":
            commands_text = (
                ...
                "🍪 НОВОЕ: КОРМЛЕНИЕ\n"
                "/feed - покормить дракона сладостями\n\n"
                
                "☕ КОФЕ И ЕДА:\n"
                "/coffee - приготовить кофе с угощением\n\n"
                ...
                "⏰ УВЕДОМЛЕНИЯ:\n"
                "• Одно утреннее уведомление в день\n"
                "• Только если не было активности сегодня\n"
                "• Время: 8-10 утра по UTC\n\n"
                
                "━━━━━━━━━━━━━━━━━━━\n"
                "💡 Также используй кнопки меню для быстрого доступа!"
            )
            ...
            characters_intro = (
                "🎭 ВСЕ ХАРАКТЕРЫ ДРАКОНОВ\n\n"
                
                "✨ Каждый дракон обладает уникальным характером,\n"
                "который влияет на его поведение, реакции и предпочтения!\n\n"
                
                "👇 Выбери характер, чтобы узнать о нём подробнее:\n\n"
                
                "• ☕ Кофеман - обожает кофе и разбирается в сортах\n"
                "• 📚 Книгочей - живёт в мире книг и историй\n"
                "• 💖 Неженка - требует много ласки и внимания\n"
                "• ✨ Чистюля - следит за чистотой и уходом\n"
                "• 🍰 Гурман - ценитель изысканных вкусов\n"
                "• 🎮 Игрик - обожает игры и соревнования\n"
                "• 😴 Соня - мастер сладких снов и отдыха\n"
                "• ⚡ Энерджайзер - живая электростанция\n"
                "• 🤔 Философ - мудрец драконьего мира\n"
                "• 🔍 Исследователь - искатель тайн и загадок\n\n"
                
                "💡 Характер определяется при создании дракона\n"
                "и остаётся с ним на всю жизнь!"
            )
...
async def process_character_detail(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        character_text += (
            "━━━━━━━━━━━━━━━━━━━\n"
            "💡 Этот характер будет влиять на все действия дракона,\n"
            "его реакции в уведомленияи и предпочтения в еде и уходе!"
        )
...
async def cmd_create(message: types.Message, state: FSMContext):
    try:
        ...
        await state.clear()
        await message.answer("❌ Произошла ошибка при создании дракона.", parse_mode="HTML")
...
async def process_dragon_name(message: types.Message, state: FSMContext):
    try:
        ...
        if not success:
            await message.answer("❌ Не удалось создать дракона. Попробуй еще раз.", parse_mode="HTML")
            await state.clear()
            return
        ...
        await message.answer(
            ...
            f"✨ Из яйца появился {escape_html(dragon_name)} - твой кофейный дракон!\n\n"
            
            f"{char_info['emoji']} ХАРАКТЕР: {char_info['name']}\n"
            f"{char_info['description']}\n\n"
...
async def cmd_status(message: types.Message):
    try:
        ...
        status_text = (
            f"{char_info['emoji']} {escape_html(dragon.name)} "
            f"[Уровень {dragon.level}]\n"
            f"🎭 Характер: {char_info['name']}\n\n"
            
            f"⭐ Опыт: {dragon.experience}/100\n"
            f"💰 Золото: {dragon.gold}\n\n"
            
            f"📊 ПОКАЗАТЕЛИ (уменьшаются на 5%/час):\n"
        )
        ...
        status_text += f"💬 {character_message}\n\n"
        ...
        status_text += (
            f"━━━━━━━━━━━━━━━━━━━\n"
            f"🕐 Текущее время (UTC): {now.strftime('%H:%M:%S')}\n"
        ...
        await message.answer("❌ Произошла ошибка при получении статуса.", parse_mode="HTML")
...
async def cmd_hug(message: types.Message):
    try:
        ...
        if not can_hug and spam_message:
            await message.answer(
                f"🤗 {spam_message}\n\n"
                f"💡 Давай подождём немного перед следующими объятиями",
                parse_mode="HTML"
            )
            return
        ...
        dragon_data = db.get_dragon(user_id)
        if not dragon_data:
            await message.answer("🐣 Сначала создай дракона!", parse_mode="HTML")
            return
        ...
        if character_trait == "неженка":
            ...
            character_bonus = "💖 Неженка обожает обнимашки! +25 к настроению, +10 к бодрости\n"
        elif character_trait == "энерджайзер":
            dragon.stats["энергия"] = min(100, dragon.stats["энергия"] + 15)
            character_bonus = "⚡ Энерджайзер заряжается от объятий! +15 к энергии\n"
        elif character_trait == "соня":
            dragon.stats["сон"] = min(100, dragon.stats["сон"] + 20)
            character_bonus = "😴 Соне в объятиях тепло и уютно! +20 к бодрости\n"
        else:
            character_bonus = ""
        ...
        await message.answer("❌ Произошла ошибка при обнимашках.", parse_mode="HTML")
...
async def cmd_coffee(message: types.Message, state: FSMContext):
    try:
        ...
        if not can_coffee and spam_message:
            await message.answer(
                f"☕ {spam_message}\n\n"
                f"💡 Давай подождём немного перед следующим кофе",
                parse_mode="HTML"
            )
            return
        ...
        if coffee_beans <= 0:
            await message.answer(
                ...
                "• Нажми «🛍️ Магазин»\n"
                "• Или /shop\n\n"
                "💡 Кофейные зёрна можно купить в категории «Кофе и ингредиенты»",
                parse_mode="HTML",
                reply_markup=get_main_keyboard()
            )
            return
        ...
        await message.answer(
            f"☕ ПРИГОТОВЬ КОФЕ ДЛЯ {escape_html(dragon.name)}\n\n"
            f"{char_message}\n\n"
            
            f"✨ Кофейный показатель: {coffee_stat}%\n\n"
            
            f"💡 Выбери напиток:\n"
            f"• ☕ Эспрессо - классический крепкий кофе\n"
            f"• ☕ Латте - с молоком и нежной пенкой\n"
            f"• ☕ Капучино - воздушная пенка и молоко\n"
            f"• ☕ Раф - с ванильным сахаром и сливками\n"
            f"• ☕ Американо - эспрессо с водой\n"
            f"• ☕ Мокко - с шоколадом и молоком\n\n"
            
            f"📦 Кофейные зёрна: {coffee_beans}\n"
            f"🎭 Характер: {character_trait}\n\n"
            
            f"Любимый кофе дракона: {dragon.favorites.get('кофе', 'латте')}",
            parse_mode="HTML",
        ...
        await message.answer("❌ Произошла ошибка при приготовлении кофе.", parse_mode="HTML")
...
async def process_coffee_choice(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        if has_additions:
            ...
            await callback.message.edit_text(
                f"☕ Выбран {get_coffee_name(action)} для {escape_html(dragon.name)}\n\n"
                f"✨ Теперь выбери добавку (или пропусти):\n\n"
                f"📦 Доступные добавки:\n"
                f"{additions_text}\n"
                f"💡 Добавки улучшают настроение дракона!",
                parse_mode="HTML",
...
async def process_coffee_additions(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        if action == "back":
            ...
            if dragon_data:
                ...
                await callback.message.edit_text(
                    ...
                    f"✨ Кофейный показатель: {dragon.stats.get('кофе', 0)}%\n\n"
                    f"💡 Выбери напиток:\n"
                    f"• ☕ Эспрессо - классический крепкий кофе\n"
                    f"• ☕ Латте - с молоком и нежной пенкой\n"
                    f"• ☕ Капучино - воздушная пенка и молоко\n"
        ...
        if has_snacks:
            ...
            # Редактируем сообщение (для удобства навигации)
            await callback.message.edit_text(
                f"☕ {get_coffee_name(data.get('coffee_type', 'espresso'))} с {get_addition_name(action)} готов!\n\n"
                f"✨ Теперь выбери сладость (или пропусти):\n\n"
                f"📦 Доступные сладости:\n"
                f"{snacks_text}\n"
                f"💡 Сладости улучшают настроение и сытость дракона!",
                parse_mode="HTML",
...
async def process_coffee_additions_no_additions(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        if has_snacks:
            ...
            # Редактируем сообщение (для удобства навигации)
            await callback.message.edit_text(
                f"☕ {get_coffee_name(data.get('coffee_type', 'espresso'))} готов!\n\n"
                f"✨ Теперь выбери сладость (или пропусти):\n\n"
                f"📦 Доступные сладости:\n"
                f"{snacks_text}\n"
                f"💡 Сладости улучшают настроение и сытость дракона!",
    ...
    try:
        ...
        if action == "back":
            ...
            if dragon_data:
                ...
                # Редактируем сообщение (для удобства навигации)
                await callback.message.edit_text(
                    f"☕ {get_coffee_name(data.get('coffee_type', 'espresso'))} с {get_addition_name(data.get('addition', 'none'))} готов!\n\n"
                    f"✨ Теперь выбери сладость (или пропусти):\n\n"
                    f"📦 Доступные сладости:\n",
                    parse_mode="HTML",
                    reply_markup=get_coffee_snack_keyboard(inventory)
...
async def finish_coffee_preparation(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        response += (
            f"\n\n━━━━━━━━━━━━━━━━━━━\n"
            f"☕ Текущий показатель кофе: {dragon.stats.get('кофе', 0)}%\n"
            f"😊 Настроение: {dragon.stats.get('настроение', 0)}%\n"
            f"🍪 Сытость: {dragon.stats.get('аппетит', 0)}%"
        )
...
async def cmd_feed(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not can_feed and spam_message:
            await message.answer(
                f"🍪 {spam_message}\n\n"
                f"💡 Давай подождём немного перед следующим кормлением",
                parse_mode="HTML"
            )
            return
        ...
        if not has_snacks:
            await message.answer(
                ...
                "💡 Сладости повышают сытость и настроение дракона!",
                parse_mode="HTML",
                reply_markup=get_main_keyboard()
            )
            return
        ...
        await message.answer(
            f"🍪 ПОКОРМИТЬ {escape_html(dragon.name)}\n\n"
            f"{char_message}\n\n"
            
            f"✨ Текущая сытость: {appetite_stat}%\n\n"
            
            f"💡 Выбери угощение:\n"
            f"Сладости повышают сытость и настроение дракона!\n\n"
            
            f"🎭 Характер: {character_trait}\n"
            f"Любимая сладость: {dragon.favorites.get('сладость', 'печенье')}",
            parse_mode="HTML",
        ...
        await message.answer("❌ Произошла ошибка при кормлении.", parse_mode="HTML")
...
async def process_feed_action(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        response += (
            f"\n\n━━━━━━━━━━━━━━━━━━━\n"
            f"🍪 Текущая сытость: {dragon.stats.get('аппетит', 0)}%\n"
            f"😊 Настроение: {dragon.stats.get('настроение', 0)}%"
        )
...
async def cmd_sleep(message: types.Message, state: FSMContext):
    try:
        ...
        if not can_sleep and spam_message:
            await message.answer(
                f"😴 {spam_message}\n\n"
                f"💡 Давай подождём немного перед следующим сном",
                parse_mode="HTML"
            )
            return
        ...
        await message.answer(
            f"😴 УЛОЖИТЬ {escape_html(dragon.name)} СПАТЬ\n\n"
            f"{char_message}\n\n"
            f"✨ Бодрость: {sleep_stat}%\n\n"
            f"💡 Выбери способ уложить дракона:\n"
            f"• 📖 Почитать сказку - убаюкать историей\n"
            f"• 💤 Лечь рядом - согреть своим теплом\n"
            f"• 😘 Поцеловать в лобик - нежный поцелуй\n"
            f"• 🎵 Спеть колыбельную - успокаивающая мелодия\n"
            f"• 🧸 Дать игрушку - для сладких снов\n"
            f"• 🌙 Просто уложить - стандартный способ\n\n"
            f"💡 Книгочею особенно понравится чтение сказки!",
            parse_mode="HTML",
        ...
        await message.answer("❌ Произошла ошибка при укладывании спать.", parse_mode="HTML")
    ...
    try:
        ...
        if action == "read":
            ...
            if character_trait == "книгочей":
                ...
                bonus_text = "📚 Книгочей обожает сказки! +20 к настроению, +15 к бодрости\n"
            else:
                bonus_text = ""
            ...
            bonus_text = "💤 Тепло хранителя: +10 к настроению\n"
            ...
            result = dragon.apply_action("сон")
            dragon.stats["настроение"] = min(100, dragon.stats.get("настроение", 0) + 15)
            bonus_text = "😘 Нежный поцелуй: +15 к настроению\n"
            scene = ActionDescriptions.get_sleep_scene(dragon.name, action)
            ...
            bonus_text = "🎵 Колыбельная: +10 к бодрости\n"
            scene = ActionDescriptions.get_sleep_scene(dragon.name, action)
            ...
            if inventory.get("plush_dragon", 0) > 0 or inventory.get("toy", 0) > 0:
                bonus_text = "🧸 С игрушкой: +20 к бодрости\n"
        ...
        response += (
            f"\n\n━━━━━━━━━━━━━━━━━━━\n"
            f"💤 Текущая бодрость: {dragon.stats.get('сон', 0)}%\n"
            f"⚡ Энергия: {dragon.stats.get('энергия', 0)}%"
        )
...
async def cmd_care(message: types.Message, state: FSMContext):
    try:
        ...
        if not can_care and spam_message:
            await message.answer(
                f"✨ {spam_message}\n\n"
                f"💡 Давай подождём немного перед следующей процедурой",
                parse_mode="HTML"
            )
            return
        ...
        care_text = (
            f"✨ УХАЖИВАТЬ ЗА {escape_html(dragon.name)}\n\n"
            f"{char_message}\n\n"
            f"✨ Показатель пушистости: {fluff_stat}%\n\n"
            f"💡 Выбери процедуру:\n"
            f"• ✨ Расчесать лапки - базовая процедура\n"
            f"• 🛁 Протереть мордочку - гигиена\n"
            f"• 💅 Почистить когти - уход за коготками\n"
            f"• 🦷 Почистить зубы - здоровье зубов\n"
        )
        
        # Добавляем дополнительные опции если есть предметы
        additional_options = ""
        if inventory.get("dragon_brush", 0) > 0:
            additional_options += "• 💆 Расчесать шерстку - с расчёской (лучший эффект)\n"
        if inventory.get("magic_shampoo", 0) > 0:
            additional_options += "• 🧴 Искупать с шампунем - полноценная ванна\n"
        if inventory.get("golden_scissors", 0) > 0:
            additional_options += "• ✂️ Подстричь когти ножницами - профессиональный уход\n"
        if inventory.get("plush_dragon", 0) > 0:
            additional_options += "• 🧸 Играть с игрушкой - развлечение и уход\n"
        ...
        care_text += (
            ...
            f"💡 Чистюля особенно оценит качественный уход!"
        )
        ...
        await message.answer("❌ Произошла ошибка при уходе.", parse_mode="HTML")
...
async def process_care_action(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        if character_trait == "чистюля":
            dragon.stats["настроение"] = min(100, dragon.stats.get("настроение", 0) + 20)
            character_bonus = "✨ Чистюля в восторге! +20 к настроению\n"
...
async def cmd_games(message: types.Message, state: FSMContext):
    try:
        ...
        if not can_game and spam_message:
            await message.answer(
                f"🎮 {spam_message}\n\n"
                f"💡 Давай подождём немного перед следующей игрой",
                parse_mode="HTML"
            )
            return
        ...
        if energy_stat < 20:
            await message.answer(
                ...
                "Ему нужен отдых или кофе. Попробуй:\n"
                "• 😴 Уложить спать\n"
                "• ☕ Приготовить кофе\n"
                "• 🤗 Просто обнять\n\n"
                "Игры требуют много энергии!",
                parse_mode="HTML",
                reply_markup=get_main_keyboard()
            )
            return
        ...
        # ОШИБКА БЫЛА ЗДЕСЬ: неправильное закрытие тегов
        # Исправленный текст:
        await message.answer(
            f"🎮 ИГРАТЬ С {escape_html(dragon.name)}\n\n"
            f"{char_message}\n\n"
            f"⚡ Энергия дракона: {energy_stat}%\n"
            f"🎭 Характер: {character_trait}\n\n"
            f"💡 Доступные игры:\n"
            f"• 🔢 Угадай число - дракон загадал число от 1 до 20\n\n"
            f"💡 Игрик будет особенно рад поиграть!",
            parse_mode="HTML",
        ...
        await message.answer("❌ Произошла ошибка при запуске игр.", parse_mode="HTML")
...
async def process_guess_number(message: types.Message, state: FSMContext):
    try:
        ...
        if not game_data or not dragon_data:
            await message.answer("❌ Ошибка в игре. Начни заново.", parse_mode="HTML")
            await state.clear()
            return
        ...
        if not guess_text.isdigit():
            await message.answer("❌ Пожалуйста, введи число!", parse_mode="HTML")
            return
        
        guess = int(guess_text)
        
        if guess < 1 or guess > 20:
            await message.answer("❌ Число должно быть от 1 до 20!", parse_mode="HTML")
            return
        ...
        if guess == secret:
            ...
            if character_trait == "игрик":
                dragon.stats["настроение"] = min(100, dragon.stats.get("настроение", 0) + 15)
                dragon.gold += 10
                character_bonus = "🎮 Игрик обожает побеждать! +15 к настроению, +10 золота\n"
            else:
                character_bonus = ""
            ...
            response = (
                f"🎉 ПОБЕДА! Ты угадал с {attempts} попытки!\n\n"
                f"Дракон загадал число {secret}\n\n"
                f"🏆 НАГРАДА:\n"
            ...
            if level_up:
                response += f"\n\n🎊 Уровень повышен! Теперь {dragon.name} {dragon.level}-го уровня!"
            
            response += f"\n\n💬 {CharacterPersonality.get_character_message(character_trait, 'win', dragon.name)}"
            ...
            if remaining_attempts <= 0:
                ...
                response = (
                    ...
                    f"Дракон загадал число {secret}\n\n"
                    f"💬 {CharacterPersonality.get_character_message(dragon.character.get('основная_черта', ''), 'lose', dragon.name)}"
                )
                ...
                response = (
                    f"❌ Не угадал!\n\n"
                    f"Число {guess} - {direction}, чем загаданное.\n\n"
                    f"{hint}\n\n"
        ...
        await message.answer("❌ Произошла ошибка в игре.", parse_mode="HTML")
...
async def cmd_shop(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            f"💰 Ваш баланс: {dragon.gold} золота\n\n"
            f"👇 Выбери категорию товаров:\n\n"
            f"• ☕ Кофе и ингредиенты - для приготовления напитков\n"
            f"• 🍪 Сладости и угощения - чтобы порадовать дракона\n"
            f"• ✨ Предметы для ухода - для красоты и здоровья\n\n"
            f"💡 Каждый предмет имеет уникальные свойства!",
            parse_mode="HTML",
        ...
        await message.answer("❌ Произошла ошибка при открытии магазина.", parse_mode="HTML")
...
async def process_shop_main(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        if action == "coffee":
            ...
            await callback.message.edit_text(
                f"☕ КОФЕ И ИНГРЕДИЕНТЫ\n\n"
                f"💰 Ваш баланс: {dragon.gold} золота\n\n"
                f"📋 Товары:\n"
                f"• ☕ Кофейные зёрна (10💰) - основной ингредиент для кофе\n"
                f"• 🍫 Шоколадные чипсы (8💰) - добавка для мокко и латте\n"
                f"• 🍯 Медовый сироп (12💰) - натуральный подсластитель\n"
                f"• 🍦 Ванильное мороженое (15💰) - для рафа и холодных напитков\n"
                f"• 🍭 Карамельный сироп (10💰) - сладкая карамельная добавка\n"
                f"• 🌰 Фундук молотый (18💰) - для ароматных напитков\n\n"
                f"💡 Кофеман особенно оценит качественные ингредиенты!",
                parse_mode="HTML",
            ...
            await callback.message.edit_text(
                f"🍪 СЛАДОСТИ И УГОЩЕНИЯ\n\n"
                f"💰 Ваш баланс: {dragon.gold} золота\n\n"
                f"📋 Товары:\n"
                f"• 🍪 Печенье с изюмом (5💰) - простое и вкусное\n"
                f"• 🍫 Шоколадная плитка (15💰) - любимое лакомство многих драконов\n"
                f"• ☁️ Ванильный зефир (7💰) - воздушный и нежный\n"
                f"• 🎄 Имбирный пряник (8💰) - с ароматными специями\n"
                f"• 🍬 Фруктовый мармелад (10💰) - яркий и вкусный\n"
                f"• 🎂 Шоколадное пирожное (20💰) - праздничное угощение\n"
                f"• 🍩 Сладкий пончик (12💰) - с сахарной пудрой\n\n"
                f"💡 Гурман разбирается в качестве сладостей!",
                parse_mode="HTML",
            ...
            await callback.message.edit_text(
                f"✨ ПРЕДМЕТЫ ДЛЯ УХОДА\n\n"
                f"💰 Ваш баланс: {dragon.gold} золота\n\n"
                f"📋 Товары:\n"
                f"• 💆 Драконья расчёска (25💰) - для идеальной шёрстки\n"
                f"• 🧴 Волшебный шампунь (30💰) - делает шерсть блестящей\n"
                f"• ✂️ Золотые ножницы (35💰) - для аккуратных коготков\n"
                f"• 🧸 Плюшевый дракончик (40💰) - лучший друг для игр\n"
                f"• 🛁 Ароматная соль (20💰) - для расслабляющих ванн\n"
                f"• 💅 Лак для когтей (28💰) - для стильного вида\n\n"
                f"💡 Чистюля обожает качественные средства для ухода!",
                parse_mode="HTML",
...
async def process_buy_item(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await callback.message.answer(
            f"✅ УСПЕШНАЯ ПОКУПКА!\n\n"
            f"Вы купили {item_id.replace('_', ' ').title()} за {price}💰\n\n"
            f"💰 Новый баланс: {dragon.gold} золота\n\n"
            f"💡 Теперь вы можете использовать этот предмет!",
            parse_mode="HTML",
            reply_markup=get_shop_main_keyboard()
        )
...
async def cmd_inventory(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            f"📦 ИНВЕНТАРЬ {escape_html(dragon.name)}\n\n"
            f"💰 Золото: {dragon.gold}\n"
            f"📊 Всего предметов: {total_items}\n\n"
            f"👇 Выбери категорию для просмотра:\n\n"
            f"• 🍪 Сладости - угощения для дракона\n"
            f"• ✨ Уход - предметы для заботы\n"
            f"• ☕ Ингредиенты - для приготовления кофе\n"
            f"• 🧸 Прочее - разные полезные вещи\n\n"
            f"💡 Предметы используются автоматически при соответствующих действиях!",
            parse_mode="HTML",
        ...
        await message.answer("❌ Произошла ошибка при открытии инвентаря.", parse_mode="HTML")
    ...
    try:
        ...
        if action in categories:
            ...
            for item_id, item_name in category_items.items():
                ...
                if count > 0:
                    items_text += f"• {item_name}: {int(count)} шт.\n"
                    total_count += count
            ...
            category_text = (
                f"{category_names[action]}\n\n"
                f"{category_desc[action]}\n\n"
                f"📦 ПРЕДМЕТЫ:\n"
                f"{items_text}\n"
                f"📊 Всего в категории: {total_count} предметов\n\n"
                f"💡 Предметы расходуются автоматически при использовании"
            )
...
async def send_smart_notifications():
    """Отправляет умные уведомления (1 раз в день утром)"""
    while True:
        try:
            now = datetime.now(timezone.utc)
            
            # Проверяем время (8-10 утра)
            if 8 <= now.hour <= 10:
                try:
                    ...
                    for user_id in users:
                        try:
                            ...
                            if smart_notifier.should_send_notification(user_id):
                                dragon_data = db.get_dragon(user_id)
                                if dragon_data:
                                    dragon = Dragon.from_dict(dragon_data)
                                    character_trait = dragon.character.get("основная_черта", "")
                                    
                                    # Получаем характерное утреннее сообщение
                                    ...
                                    # Проверяем низкие показатели для срочных уведомлений
                                    warnings = []
                                    if dragon.stats.get("кофе", 70) < 30:
                                        warnings.append("☕ Срочно нужен кофе!")
                                    if dragon.stats.get("настроение", 80) < 30:
                                        warnings.append("😔 Грустит без внимания...")
                                    if dragon.stats.get("аппетит", 50) < 30:
                                    ...
                                    if warnings:
                                        notification_text += "⚠️ ВНИМАНИЕ:\n"
                                        for warning in warnings:
                                            notification_text += f"• {warning}\n"
                                        notification_text += "\n"
                                    
                                    notification_text += (
                                        f"💡 {dragon.name} ждёт твоей заботы!\n"
                                        f"• ☕ Приготовь кофе\n"
                                        f"• 🤗 Обними\n"
                                        f"• 🍪 Покорми\n\n"
                                    ...
                                    # Записываем отправленное уведомление
                                    smart_notifier.record_notification(user_id)
            ...
            smart_notifier.clear_old_entries()
            ...
            logger.error(f"Ошибка в send_smart_notifications: {e}")
...
async def main():
    try:
        ...
        # Запускаем задачу для умных уведомлений
        asyncio.create_task(send_smart_notifications())
