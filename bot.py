"""
🐉 КОФЕЙНЫЙ ДРАКОН - Версия 3.1
Исправлены ошибки экранирования MarkdownV2
"""
import asyncio
import logging
import random
...
async def cmd_start(message: types.Message):
    ...
    try:
        ...
        if has_dragon:
            welcome_text += escape_markdown(f"У тебя уже есть дракон! 🎉\n")
            welcome_text += escape_markdown(f"Используй кнопку «🐉 Статус» чтобы проверить как он поживает.")
            await message.answer(welcome_text, parse_mode="MarkdownV2", reply_markup=get_main_keyboard())
        else:
            welcome_text += escape_markdown(f"Нажми «🐉 Создать дракона» чтобы начать приключение!")
...
async def process_dragon_name(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not is_valid:
            await message.answer(
                escape_markdown(f"❌ {error_message}\n\nПопробуй другое имя:"),
                parse_mode="MarkdownV2"
            )
            return
...
async def cmd_status(message: types.Message):
    ...
    try:
        ...
        if warnings:
            status_text += escape_markdown("⚠️ ВНИМАНИЕ:\n")
            for warning in warnings:
                status_text += escape_markdown(f"• {warning}\n")
            status_text += escape_markdown("\n")
...
async def cmd_inventory(message: types.Message):
    ...
    try:
        ...
        items_text = escape_markdown("📦 ТВОЙ ИНВЕНТАРЬ\n\n")
        ...
        for item, count in inventory.items():
            if item in ["кофейные_зерна", "вода"]:
                ...
                coffee_items.append(escape_markdown(f"• {emoji} {name}: {count}"))
        
        if coffee_items:
            items_text += escape_markdown("☕ КОФЕ И НАПИТКИ:\n") + "\n".join(coffee_items) + escape_markdown("\n\n")
        ...
        for item in snacks:
            ...
            if count > 0:
                ...
                snack_items.append(escape_markdown(f"• {emoji} {name}: {count}"))
        
        if snack_items:
            items_text += escape_markdown("🍬 СЛАДОСТИ:\n") + "\n".join(snack_items) + escape_markdown("\n\n")
        
        # Игры и развлечения
        game_items = []
        if inventory.get("игральная_кость", 0) > 0:
            game_items.append(escape_markdown(f"• 🎲 Игральная кость: {inventory.get('игральная_кость', 0)}"))
        
        if game_items:
            items_text += escape_markdown("🎮 ИГРЫ И РАЗВЛЕЧЕНИЯ:\n") + "\n".join(game_items) + escape_markdown("\n\n")
        
        items_text += escape_markdown(f"💰 ЗОЛОТО: {gold}\n\n")
        items_text += escape_markdown("━━━━━━━━━━━━━━━━━━━\n")
        items_text += escape_markdown("Используй сладости для кормления дракона! 🐾")
...
async def cmd_gold(message: types.Message):
    ...
    try:
        ...
        await message.answer(
            escape_markdown(random.choice(responses)), 
            parse_mode="MarkdownV2", 
            reply_markup=get_main_keyboard()
        )
