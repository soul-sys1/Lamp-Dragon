"""
ГЛАВНЫЙ ФАЙЛ БОТА - КОФЕЙНЫЙ ДРАКОН
Обрабатывает все команды и сообщения
Версия 2.0 - Исправлены ошибки и оптимизирована работа
"""
import asyncio
import logging
import random
...
def get_shop_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура магазина (кешированная)"""
...
def get_reading_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура для чтения (кешированная)"""
...
def get_coffee_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура для приготовления кофе (кешированная)"""
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
...
async def cmd_start(message: types.Message):
    ...
    try:
        ...
        welcome_text = (
            "🐉 **Добро пожаловать в мир Кофейных Драконов!**\n\n"
...
async def cmd_create(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при создании дракона")
...
async def process_dragon_name(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not is_valid:
            await message.answer(f"{error_message} Попробуй еще:")
            return
...
async def cmd_coffee(message: types.Message):
    ...
    try:
        ...
        if not rate_limiter.can_perform_action(user_id, "coffee", 10):
            await message.answer("Подожди немного перед следующим кофе ☕")
            return
        ...
        await message.answer("Произошла ошибка при приготовлении кофе")
...
async def cmd_feed(message: types.Message):
    ...
    try:
        ...
        if not rate_limiter.can_perform_action(user_id, "feed", 15):
            await message.answer("Дракон еще не проголодался. Подожди немного 🍪")
            return
        ...
        await message.answer("Произошла ошибка при кормлении")
...
async def cmd_hug(message: types.Message):
    ...
    try:
        ...
        if not rate_limiter.can_perform_action(user_id, "hug", 5):
            await message.answer("Не переусердствуй с объятиями! Подожди немного 🤗")
            return
        ...
        await message.answer("Произошла ошибка при обнимашках")
...
async def cmd_read(message: types.Message):
    ...
    try:
        ...
        if not rate_limiter.can_perform_action(user_id, "read", 30):
            await message.answer("Дракону нужно время чтобы осмыслить прочитанное. Подожди немного 📚")
            return
        ...
        await message.answer("Произошла ошибка при чтении")
...
async def cmd_play(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not rate_limiter.can_perform_action(user_id, "play", 20):
            await message.answer("Дракон устал от игр. Дайте ему отдохнуть 🎮")
            return
...
async def cmd_clean(message: types.Message):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при уходе")
...
async def cmd_inventory(message: types.Message):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при просмотре инвентаря")
...
async def cmd_gold(message: types.Message):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при проверке золота")
...
async def cmd_rename(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при переименовании")
...
async def cmd_achievements(message: types.Message):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при получении достижений")
