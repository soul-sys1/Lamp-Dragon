...
Теперь показатели уменьшаются на 10% в час
"""
import random
...
class Dragon:
    def __init__(self, name="Дракоша"):
        ...
        self.stats = {
            "кофе": 70,          # Хочет кофе
            "сон": 30,           # Хочет спать
            "настроение": 80,    # Общее настроение
            "аппетит": 60,       # Голод
            "энергия": 75,       # Энергия для активностей
            "пушистость": 90,    # Ухоженность шерсти
            "чистота": 85,       # Общая чистота
    ...
    def _generate_character(self) -> Dict:
        ...
        traits = [
            ...
            "лентяй",       # Не любит активность, предпочитает отдых
            "энерджайзер",  # Всегда полон энергии, активен
            "философ",      # Любит размышлять, спокойный
            "артист",       # Творческая натура, любит искусство
            "спортсмен",    # Активный, любит движение
            "исследователь",# Любознательный, изучает всё вокруг
            "дружелюбный",  # Общительный, любит компанию
            "мечтатель"     # Витает в облаках, фантазёр
        ]
    ...
    def _get_trait_description(self, trait: str) -> str:
        ...
        descriptions = {
            "кофеман": "Обожает кофе и разбирается в его сортах. Может часами говорить о разных способах заварки.",
            "соня": "Любит поспать и вздремнуть. Часто клюёт носом во время чтения или игр.",
            "игрик": "Обожает игры и соревнования. Всегда готов к новым вызовам и победам.",
            "книгочей": "Проводит дни за чтением. Знает множество историй и легенд о драконах.",
            "неженка": "Требует много ласки и внимания. Обожает объятия и нежные слова.",
            "гурман": "Разбирается в еде и напитках. Ценит качество и изысканность вкусов.",
            "чистюля": "Следит за своей чистотой. Любит, когда его расчёсывают и ухаживают за ним.",
            "лентяй": "Не любит лишнюю активность. Предпочитает лежать на мягкой подушке.",
            "энерджайзер": "Всегда полон энергии. Готов бегать и играть целый день.",
            "философ": "Любит размышлять о жизни. Часто задаёт глубокие вопросы.",
            "артист": "Творческая натура. Любит рисовать, петь и создавать красивые вещи.",
            "спортсмен": "Активный и подвижный. Любит соревнования и физические нагрузки.",
            "исследователь": "Любознательный и умный. Изучает всё вокруг с большим интересом.",
            "дружелюбный": "Общительный и весёлый. Любит компании и новые знакомства.",
            "мечтатель": "Витает в облаках. Часто погружён в свои фантазии и мечты."
        }
    ...
    def _generate_favorites(self) -> Dict:
        ...
        trait_preferences = {
            "кофеман": {"кофе": "эспрессо", "активность": "дегустация кофе"},
            "соня": {"активность": "сон", "время_суток": "ночь"},
            "игрик": {"активность": "игры", "сладость": random.choice(["пончик", "кекс"])},
            "книгочей": {"активность": "чтение", "жанр_книг": "фэнтези"},
            "неженка": {"активность": "обнимашки", "цвет": "розовый"},
            "гурман": {"сладость": "пирожное", "кофе": "мокко"},
            ...
            "дружелюбный": {"активность": "общение", "время_суток": "вечер"},
            "мечтатель": {"активность": "мечтания", "погода": "облачно"}
        }
        
        if main_trait in trait_preferences:
            favorites.update(trait_preferences[main_trait])
        
        # ДОПОЛНИТЕЛЬНЫЕ ПРЕДПОЧТЕНИЯ ОТ ВТОРОСТЕПЕННЫХ ЧЕРТ
        secondary_traits = self.character.get("второстепенные", [])
        for trait in secondary_traits[:2]:  # Берем максимум 2 дополнительные черты
            if trait in trait_preferences:
                for key, value in trait_preferences[trait].items():
                    if random.random() > 0.7:  # 30% шанс принять предпочтение
                        favorites[key] = value
        
        return favorites
    
    def update_over_time(self):
        """Обновляет показатели со временем - УМЕНЬШАЕТСЯ НА 10% В ЧАС"""
        try:
            now = datetime.now()
            ...
            # УМЕНЬШЕНИЕ ПОКАЗАТЕЛЕЙ НА 10% В ЧАС
            # Для кофе, энергии, настроения, пушистости, чистоты, здоровья
            decay_rates = {
                "кофе": 10.0,           # 10% в час
                "энергия": 10.0,        # 10% в час
                "настроение": 10.0,     # 10% в час
                "пушистость": 10.0,     # 10% в час
                "чистота": 10.0,        # 10% в час
                "здоровье": 10.0        # 10% в час
            }
            
            growth_rates = {
                "сон": 10.0,            # Увеличивается на 10% в час
                "аппетит": 10.0,        # Увеличивается на 10% в час
            }
        ...
        try:
            ...
            if action_type in base_effects:
                ...
                if action_type == "кофе" and action_data.get("addition"):
                    ...
                    if addition != "none":
                        self.stats["настроение"] = min(100, self.stats["настроение"] + 5)
                        result["stat_changes"]["настроение"] = result["stat_changes"].get("настроение", 0) + 5
                
                # Бонусы за сладости с кофе
                if action_type == "кофе" and action_data.get("snack"):
                    snack = action_data.get("snack")
                    if snack != "none":
                        self.stats["настроение"] = min(100, self.stats["настроение"] + 10)
                ...
                character_bonuses = {
                    ...
                    "неженка": {"action": "обнимашки", "bonus": {"настроение": 20, "сон": -10}},
                    "игрик": {"action": "игра", "bonus": {"настроение": 20, "игровая_эрудиция": 5}},
                    "книгочей": {"action": "сон", "bonus": {"интеллект": 5, "литературный_вкус": 5}},
                    "чистюля": {"action": "уход", "bonus": {"настроение": 25, "пушистость": 15}},
                    "гурман": {"action": "кормление", "bonus": {"настроение": 20, "кулинарное_искусство": 5}},
                    "соня": {"action": "сон", "bonus": {"сон": -20, "энергия": 30}},
    ...
    def apply_minigame_result(self, game_type: str, won: bool, score: int = 0) -> Dict:
        ...
        try:
            ...
            if self.character.get("основная_черта") == "игрик":
                ...
                if won:
                    result["message"] += "\n🎮 Игрик в восторге от победы!"
                else:
                    result["message"] += "\n🎮 Игрик немного расстроен поражением..."
    ...
    def can_perform_action(self, action_type: str) -> tuple[bool, str]:
        ...
        try:
            ...
            checks = {
                "кофе": lambda: (
                    self.stats["кофе"] < 95,
                    f"☕ {self.name} уже полон кофе! Подожди немного."
                ),
                "кормление": lambda: (
                    self.stats["аппетит"] > 5,
                    f"🍪 {self.name} слишком сыт! Подожди, пока проголодается."
                ),
                "обнимашки": lambda: (
                    self.stats["настроение"] < 95,
                    f"🤗 {self.name} уже на седьмом небе от счастья!"
                ),
                "уход": lambda: (
                    self.stats["пушистость"] < 95 or self.stats["чистота"] < 95,
                    f"✨ {self.name} уже идеально ухожен!"
                ),
                "сон": lambda: (
                    self.stats["сон"] > 5,
                    f"😴 {self.name} совсем не хочет спать!"
                ),
                "игра": lambda: (
                    self.stats["энергия"] > 20,
    ...
    def _get_needs_attention(self) -> List[str]:
        ...
        if self.stats["аппетит"] > 70:
            needs.append("🍪 Еда")
        if self.stats["настроение"] < 40:
            needs.append("😊 Внимание")
        if self.stats["энергия"] < 30:
            needs.append("⚡ Отдых")
        if self.stats["пушистость"] < 50:
            needs.append("✨ Уход")
        if self.stats["чистота"] < 50:
            needs.append("🛁 Чистота")
        if self.stats["здоровье"] < 70:
